...
- `bundle install && pnpm install` — install Ruby and Node dependencies from the root.
- `bin/rails db:setup` — bootstrap the database schema and seeds.
- `bundle exec rails server -p 3000` — run the backend; use `pnpm run serve` to mount the Angular dev shell alongside it.
...
- `cd frontend && pnpm test` — launch Karma/Jasmine specs (`--watch` keeps them running).
